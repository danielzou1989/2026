...
class BitgetWebSocket:
    ...
    def subscribe_candle(self, symbol: str, timeframe: str, callback: Callable):
        """
        订阅K线数据

        Args:
            symbol: 交易对符号（如BTCUSDT）
            timeframe: 时间周期（如 5m, 15m, 1H, 1D）
            callback: 数据回调函数
        """
    ...
    def subscribe_orders(self, callback: Callable):
        ...
        if not self.is_private:
            raise ValueError("Orders channel requires private WebSocket")
        ...
        self.subscriptions.append(subscription)
        self.callbacks["orders"] = callback
    ...
    def subscribe_positions(self, callback: Callable):
        ...
        """
        if not self.is_private:
            raise ValueError("Positions channel requires private WebSocket")
...
class BitgetKlineStream:
    ...
    def _on_candle(self, symbol: str, data: List[Dict]):
        """K线数据回调"""
        try:
            for candle in data:
                # Bitget K线数据格式:
                ...
                if self.data_manager:
                    self.data_manager.update_kline(symbol, kline)

                logger.debug(f"Received {symbol} candle: {kline['close']}")
    ...
    def start(self):
        ...
        for symbol in self.symbols:
            callback = lambda data, s=symbol: self._on_candle(s, data)
            self.ws.subscribe_candle(symbol, self.timeframe, callback)

        logger.info(f"Started K-line stream for {self.symbols} ({self.timeframe})")

    def stop(self):
        """停止数据流"""
        self.ws.disconnect()
        logger.info("Stopped K-line stream")
